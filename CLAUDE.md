...
- `app/page.tsx`'s "Tres pasos" step 3 used to pitch a "Publicá con un click" / activate-"Public" toggle — the app has never had one (portfolios are `published: true` from creation, no draft/private mode, see top of this file). Rewritten to describe what's actually true (nothing left to do after signing in and editing) and given each step an inline SVG icon in a lime circular badge, a `01`/`02`/`03` step number, and a `group hover:bg-white/[0.03]` hover state — matching the existing dark/lime marketing aesthetic rather than introducing a new one.
