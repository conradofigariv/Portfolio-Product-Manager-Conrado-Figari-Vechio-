...
A field-by-field replacement of the plain-text system above with rich text (bold/italic/underline/strike, font size, color, highlight, link, alignment), landing in `portfolio_blocks` instead of `portfolios.content`. Migrated so far: the original 15 top-level scalar fields (a title, a tagline, a name — see 0005), project narrative lines (see 0006, the first list-shaped field), a scalar field but keyed by a *dynamic* parent id rather than a fixed path — chapter tag/heading/body (`journey.chapters.<chapterId>.tag`/`.heading`/`.body`) and project title (`projects.items.<projectId>.title`), see 0007 — and two more plain string lists, project tags (`projects.items.<projectId>.tags.<tagId>`) and `contact.availableItems.<itemId>` (the latter not nested under any per-entity array at all — there's only one contact section per portfolio), see 0008. Still on the old plain-text system: metrics, skills categories, certs, contact socials — each a list of *multi-field* objects (label+value, title+issuer, label+url), which the single-field `useBlockList` pattern below doesn't cover yet; skills categories additionally need a stable id per category before they can be addressed by block_key at all (unlike projects/chapters, `SkillCategory` has never had one). `contact.email` was deliberately *not* migrated despite being scalar — `Contact.tsx` uses it to build a `mailto:` link and a visibility check, not just to display text, which doesn't fit a rich text field.
...
- A list needing its own hook call (`useBlockList`) can't be inlined inside a parent's `.map()` over a dynamic-length array (projects, in this case) — that violates the Rules of Hooks, since the number of hook calls would vary between renders. `ProjectNarrative.tsx` exists as its own component for exactly this reason; the same will be true for whatever component ends up owning tags/metrics/etc.
