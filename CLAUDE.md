...
- **`app/components/LandingPage.tsx`** — the landing page's own EN/ES bilingual layer, separate from `LanguageContext`. `LanguageContext` holds one portfolio owner's *content*, fetched per-user; the landing page has no portfolio and no signed-in user yet, so it can't use that context — it's a small client component with its own `COPY = { es: {...}, en: {...} }` table and a local `useState<Lang>('es')` toggled by a `🇪🇸 ES / 🇺🇸 EN` button in the header (no persistence — resets on reload, same as a fresh visit). `app/page.tsx` stays a Server Component (does the `getUser()`/`redirect('/admin')` check, reads `searchParams` for the OAuth error/reason) and passes `error`/`reason` down as props; `LandingPage` owns everything visual and interactive below that.
- The landing header is a `grid grid-cols-[1fr_auto_1fr]` (lang toggle in the left `1fr` cell via `justify-self-start`, logo+"Portfolio App" in the `auto` center cell via `justify-self-center`, auth buttons in the right `1fr` cell via `justify-self-end`) rather than `flex justify-between` — a flex row can't center the middle item independent of how wide the two side groups are; the grid's two `1fr` tracks stay equal regardless of their content, so the center cell is genuinely centered on the page, not just centered *between* the other two.
...
- The portfolio `Navbar`'s language toggle, `BackgroundPicker` trigger, and "Cerrar sesión" button are deliberately kept at identical `text-xs font-mono` + `border-dark-400/40` styling — the lang toggle used to be `text-sm` with a different border color (`border-dark-600`), visibly mismatched against the other two header pills sitting right next to it (reported: language selector looked bigger/bolder than "Cerrar sesión"). Both the desktop and mobile lang toggles now carry `🇪🇸`/`🇺🇸` flag emoji alongside the `ES`/`EN` label, matching the landing page's own toggle.
