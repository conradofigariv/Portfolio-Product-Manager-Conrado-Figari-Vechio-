...
A field-by-field replacement of the plain-text system above with rich text (bold/italic/underline/strike, font size, color, highlight, link, alignment), landing in `portfolio_blocks` instead of `portfolios.content`. Migrated so far: the original 15 top-level scalar fields (a title, a tagline, a name — see 0005), project narrative lines (see 0006, the first list-shaped field), and — a scalar field but keyed by a *dynamic* parent id rather than a fixed path — chapter tag/heading/body (`journey.chapters.<chapterId>.tag`/`.heading`/`.body`) and project title (`projects.items.<projectId>.title`), see 0007. Still on the old plain-text system: tags, metrics, skills categories, certs, contact socials/availableItems — everything still a plain owner-addable/removable list of its own. `contact.email` was deliberately *not* migrated despite being scalar — `Contact.tsx` uses it to build a `mailto:` link and a visibility check, not just to display text, which doesn't fit a rich text field.
