...
- **`app/components/LandingPage.tsx`** — the landing page's own EN/ES bilingual layer, separate from `LanguageContext`. `LanguageContext` holds one portfolio owner's *content*, fetched per-user; the landing page has no portfolio and no signed-in user yet, so it can't use that context — it's a small client component with its own `COPY = { es: {...}, en: {...} }` table and a local `useState<Lang>('es')` toggled by a `🇺🇸 EN / 🇪🇸 ES` button in the header (no persistence — resets on reload, same as a fresh visit). `app/page.tsx` stays a Server Component (does the `getUser()`/`redirect('/admin')` check, reads `searchParams` for the OAuth error/reason) and passes `error`/`reason` down as props; `LandingPage` owns everything visual and interactive below that.
