...
A field-by-field replacement of the plain-text system above with rich text (bold/italic/underline/strike, font size, color, highlight, link, alignment), landing in `portfolio_blocks` instead of `portfolios.content`. The 15 scalar fields (a title, a tagline, a name — see 0005) plus **project narrative lines** (see 0006, the first list-shaped field) are migrated. Everything else that's still a plain owner-addable/removable list — tags, metrics, skills categories, certs, contact socials/availableItems, chapter fields — is unaffected and still runs entirely on the system above. `contact.email` was deliberately *not* migrated despite being scalar — `Contact.tsx` uses it to build a `mailto:` link and a visibility check, not just to display text, which doesn't fit a rich text field.
